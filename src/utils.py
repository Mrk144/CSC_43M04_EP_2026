...
def build_transforms(image_size: int=224, is_training: bool=True, aug_cfg: object | None=None) -> transforms.Compose:
    ...
    if aug_cfg is not None and is_training:
        ...
        if use_jitter:
            jitter_brightness = float(aug_cfg.get('color_jitter', {}).get('brightness', jitter_brightness))
            jitter_contrast = float(aug_cfg.get('color_jitter', {}).get('contrast', jitter_contrast))
            jitter_saturation = float(aug_cfg.get('color_jitter', {}).get('saturation', jitter_saturation))
            jitter_hue = float(aug_cfg.get('color_jitter', {}).get('hue', jitter_hue))
        if use_erase:
            erase_p = float(aug_cfg.get('random_erasing', {}).get('prob', erase_p))
        if use_rrc:
            rrc_scale = tuple(aug_cfg.get('random_resized_crop', {}).get('scale', list(rrc_scale)))
    if is_training:
        steps = [transforms.Lambda(lambda x: tv_tensors.Video(x))]
        if use_rrc:
            steps.append(transforms.RandomResizedCrop(size=(image_size, image_size), scale=rrc_scale, ratio=(3.0 / 4.0, 4.0 / 3.0), antialias=True))
        else:
            steps.extend([transforms.Resize(size=resize_size, antialias=True), transforms.CenterCrop(size=(image_size, image_size))])
        if use_jitter:
            steps.append(transforms.ColorJitter(brightness=jitter_brightness, contrast=jitter_contrast, saturation=jitter_saturation, hue=jitter_hue))
    ...
    return transforms.Compose([transforms.Lambda(lambda x: tv_tensors.Video(x)), transforms.Resize(size=resize_size, antialias=True), transforms.CenterCrop(size=(image_size, image_size)), transforms.ToDtype(torch.float32, scale=True), transforms.Normalize(mean=mean, std=std)])
...
def apply_horizontal_flip_video(video: torch.Tensor, label: int, *, prob: float) -> Tuple[torch.Tensor, int]:
...
def mixup_batch(video: torch.Tensor, labels: torch.Tensor, *, alpha: float, num_classes: int, label_smoothing: float=0.0) -> Tuple[torch.Tensor, torch.Tensor]:
    if video.size(0) < 2:
...
def cutmix_batch(video: torch.Tensor, labels: torch.Tensor, *, alpha: float, num_classes: int, label_smoothing: float=0.0) -> Tuple[torch.Tensor, torch.Tensor]:
...
def _labels_to_soft(labels: torch.Tensor, num_classes: int, label_smoothing: float) -> torch.Tensor:
    soft = F.one_hot(labels.long(), num_classes=num_classes).float()
    if label_smoothing > 0.0:
        soft = soft * (1.0 - label_smoothing) + label_smoothing / num_classes
    return soft

def maybe_batch_augment(video: torch.Tensor, labels: torch.Tensor, aug_cfg: DictConfig, *, num_classes: int, label_smoothing: float) -> Tuple[torch.Tensor, torch.Tensor, Optional[torch.Tensor]]:
    ...
    if cutmix_on and r < cut_prob:
        (mixed, soft) = cutmix_batch(video, labels, alpha=_cfg_float(aug_cfg, 'cutmix', 'alpha', default=1.0), num_classes=num_classes, label_smoothing=label_smoothing)
        return (mixed, labels, soft)
    if mixup_on and r < mix_prob:
        (mixed, soft) = mixup_batch(video, labels, alpha=_cfg_float(aug_cfg, 'mixup', 'alpha', default=0.2), num_classes=num_classes, label_smoothing=label_smoothing)
        return (mixed, labels, soft)
    return (video, labels, None)
...
def compute_loss(loss_fn: nn.Module, logits: torch.Tensor, labels: torch.Tensor, soft_targets: Optional[torch.Tensor]) -> torch.Tensor:
...
@torch.no_grad()
def forward_logits_with_tta(model: nn.Module, video: torch.Tensor, aug_cfg: DictConfig, *, use_autocast: bool=False) -> torch.Tensor:
...
@torch.no_grad()
def accuracy_topk(logits: torch.Tensor, targets: torch.Tensor, topk: Tuple[int, ...]=(1, 5)) -> Tuple[torch.Tensor, ...]:
    max_k = max(topk)
...
def split_train_val(samples: List[Tuple[Path, int]], val_ratio: float, seed: int) -> Tuple[List[Tuple[Path, int]], List[Tuple[Path, int]]]:
...
def count_class_frequencies(samples: List[Tuple[Path, int]], num_classes: int) -> torch.Tensor:
    counts = torch.zeros(num_classes, dtype=torch.float64)
    for (_path, label) in samples:
        if not 0 <= label < num_classes:
            raise ValueError(f'label {label} out of range [0, {num_classes}) for {len(samples)} train samples')
        counts[label] += 1.0
    return counts

def class_weights_from_counts(counts: torch.Tensor, mode: str='inverse_freq', beta: float=0.9999, max_weight_ratio: float=50.0) -> torch.Tensor:
    if mode == 'effective_num' and (not 0.0 < beta < 1.0):
        raise ValueError('class_weights beta must be in (0, 1) for effective_num mode')
    ...
    if mode == 'inverse_freq':
        ...
        raise ValueError(f"Unknown class_weights_mode {mode!r}; expected 'inverse_freq' or 'effective_num'")
