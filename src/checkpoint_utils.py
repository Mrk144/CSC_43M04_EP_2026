...
def expert_eval_settings(raw: Dict[str, Any], cfg: DictConfig, expert_entry: Optional[Dict[str, Any]]=None) -> ExpertEvalSettings:
    ...
    num_frames = int(raw.get('num_frames', saved.get('dataset', {}).get('num_frames', cfg.dataset.num_frames)))
    image_size = 224
    ...
    return {'num_frames': num_frames, 'image_size': image_size, 'batch_size': batch_size, 'model_name': model_name}
...
def _cfg_for_checkpoint_load(cfg: DictConfig, state_dict: Dict[str, torch.Tensor]) -> DictConfig:
    ...
    if _vjepa_checkpoint_is_legacy_encoder(state_dict):
        ...
        cfg = OmegaConf.merge(cfg, {'model': {'variant': variant, 'pretrained': False}})
        print('V-JEPA2: loading legacy checkpoint (backbone + linear head).', flush=True)
    else:
        print('V-JEPA2: loading HuggingFace classification checkpoint (model.*).', flush=True)
    return cfg
...
def _load_state_dict_compat(model: nn.Module, state_dict: Dict[str, torch.Tensor]) -> None:
    ...
    if skipped:
        ...
        print(f'  Checkpoint keys not loaded ({len(skipped)}): {preview}{extra}', flush=True)
    if missing:
        critical = [k for k in missing if 'classifier' in k or 'fc' in k]
        if critical:
            raise RuntimeError('Checkpoint missing required head weights: ' + ', '.join(critical[:8]))
        print(f'  Model submodules initialized fresh ({len(missing)} missing keys).', flush=True)
...
def load_model_from_checkpoint(checkpoint: Dict[str, Any], device: torch.device) -> torch.nn.Module:
    ...
    if saved_cfg is None:
        raise ValueError("Checkpoint has no 'config' (or legacy 'cfg') entry. Train with the current train.py so the full Hydra config is saved with the weights.")
