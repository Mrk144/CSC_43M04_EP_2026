"""Shared MoE (logit-level) evaluation: mean or val_acc combination."""
...
def combine_logits(
    ...
    """Combine expert logits. Supported: mean | val_acc."""
    if combination == "mean":
...
def run_moe_evaluation(cfg: DictConfig) -> None:
    """Evaluate a mixture of experts on dataset.val_dir."""
    moe_cfg = cfg.moe
    ...
    for entry in experts_cfg:
        ...
        if num_classes_ref is None:
            ...
            raise ValueError(
                f"Expert {name}: num_classes={nc} != {num_classes_ref} "
...
def _expert_logits_on_samples(
    ...
    *,
    ...
    """Forward one expert; optional temperature from val (fit or ``fixed_temperature``)."""
    if not ckpt_path.is_file():
        raise FileNotFoundError(f"Expert {name}: checkpoint not found: {ckpt_path}")
...
def run_moe_submission(
    ...
    """
    MoE predictions on test clips: stack expert logits, combine, return argmax.

    - ``mean``: average test logits (equal weights).
    - ``val_acc``: fit expert weights on ``dataset.val_dir`` (top-1 per expert),
      then apply the same weights to test logits (labels not needed on test).
    """
    moe_cfg = cfg.moe
    experts_cfg = list(moe_cfg.experts)
    if len(experts_cfg) < 2:
    ...
    print(
        f"MoE submission: {len(test_samples)} test clips, combination={combination}",
        flush=True,
    )
    ...
    for entry in experts_cfg:
        ...
        if need_val and val_samples:
            ...
            val_logits = _expert_logits_on_samples(
                cfg,
                ckpt_path,
                name,
                val_samples,
            ...
            if val_labels_ref is None:
                val_labels_ref = torch.tensor(
                    [lab for _, lab in val_samples], dtype=torch.long
                )
            if calibrate:
                temperature = find_temperature(val_logits, val_labels_ref)
                print(
                    f"  [{name}] temperature T={temperature:.3f} (from val)",
                    flush=True,
                )
        ...
        if calibrate and need_val and val_samples:
            test_logits = _expert_logits_on_samples(
                cfg,
                ckpt_path,
                name,
                test_samples,
