...
try:
    ...
    raise ImportError("efficientformer_bilstm requires the 'timm' package. Install with: uv add timm") from e

class EfficientFormerBiLSTM(nn.Module):

    def __init__(self, num_classes: int, pretrained: bool=False, num_frames: int=7, variant: str='efficientformerv2_s1', lstm_hidden_size: int=256, dropout_p: float=0.5, backbone_chunk_size: int=28) -> None:
        super().__init__()
        self.num_frames = int(num_frames)
        self.backbone_chunk_size = max(1, int(backbone_chunk_size))
        self.variant = variant
        self.backbone = timm.create_model(variant, pretrained=pretrained, num_classes=0, global_pool='avg')
        ...
        self.lstm = nn.LSTM(input_size=feat_dim, hidden_size=lstm_hidden_size, num_layers=1, batch_first=True, bidirectional=True)
        pooled_dim = 2 * lstm_hidden_size
        self.attn_pool = AttentionPool1d(pooled_dim)
        self.classifier = nn.Sequential(nn.Dropout(p=dropout_p), nn.Linear(pooled_dim, num_classes))
    ...
    def _encode_frames(self, frames: torch.Tensor) -> torch.Tensor:
        ...
        return torch.cat([self.backbone(frames[i:i + chunk]) for i in range(0, n, chunk)], dim=0)
