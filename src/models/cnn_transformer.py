...
The spatial pooling level is configurable:
    - ``spatial_tokens_side=1`` (default): 1 token per frame, 8 tokens total
      including CLS. Lightest, regularises strongest. Best fit for the small
      training set.
    - ``spatial_tokens_side=2``: 4 tokens per frame, 29 total. More spatial
      capacity, still cheap.
    - ``spatial_tokens_side=0``: keep native spatial map (h*w tokens per frame,
      typically 49). Closer to TimeSformer; only use with strong augmentation.
...
class CNNTransformer(nn.Module):
    ...
    def __init__(
        ...
        if self.spatial_tokens_side > 0:
            ...
            # Pre-allocate spatial pos for the typical ResNet-18 grid at 224x224
            # (7 x 7 = 49 tokens). If the actual grid differs at forward time
            # (smaller input -> smaller grid), we resize with F.interpolate the
            # same way we do for temporal_pos.
    ...
    def forward(self, video_batch: torch.Tensor) -> torch.Tensor:
        ...
        if self.spatial_pos is not None:
            ...
            if spatial_pos.shape[2] != s:
                # Heavy mode (``spatial_tokens_side=0``) at non-default H/W:
                # resize the learned spatial embedding from its allocated grid
                # (default sqrt(49) x sqrt(49) = 7x7) to the current grid.
                ...
                if side * side == spatial_pos.shape[2] and target_side * target_side == s:
                    grid = spatial_pos.permute(0, 3, 1, 2).reshape(
                        1, feat_c, 1, side, side
                    )[:, :, 0]
                    grid = F.interpolate(
                        grid, size=(target_side, target_side),
