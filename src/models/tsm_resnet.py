...
def install_temporal_shift(resnet: nn.Module, num_frames: int, fold_div: int=8) -> None:
    for layer in (resnet.layer1, resnet.layer2, resnet.layer3, resnet.layer4):
        for block in layer:
            block.conv1 = TemporalShiftWrapper(block.conv1, num_frames=num_frames, fold_div=fold_div)

class TSMResNet(nn.Module):

    def __init__(self, num_classes: int, num_frames: int=4, pretrained: bool=False, dropout_p: float=0.5, fold_div: int=8) -> None:
        super().__init__()
        self.num_frames = num_frames
