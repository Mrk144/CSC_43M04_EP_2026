...
def _row_normalize(cm: np.ndarray) -> np.ndarray:
    ...
    return np.divide(cm.astype(np.float64), support, out=np.zeros_like(cm, dtype=np.float64), where=support > 0)

def _collect_predictions(model: torch.nn.Module, data_loader: DataLoader, device: torch.device) -> Tuple[np.ndarray, np.ndarray]:
    ...
    return (torch.cat(y_true_list).numpy().astype(np.int64), torch.cat(y_pred_list).numpy().astype(np.int64))

def _print_metrics_table(precision: np.ndarray, recall: np.ndarray, support: np.ndarray, cm: np.ndarray) -> None:
    total = float(cm.sum())
    ...
    print(f'Macro avg precision/recall (classes with support): {macro_p:.4f} / {macro_r:.4f}')
    print(f'Weighted avg precision/recall (by true-class support): {weighted_p:.4f} / {weighted_r:.4f}')
    print('')
    print(f"{'class':>5}  {'support':>8}  {'precision':>9}  {'recall':>7}")
    print('-' * 40)
    for c in range(len(support)):
        print(f'{c:5d}  {int(support[c]):8d}  {precision[c]:9.4f}  {recall[c]:7.4f}')
...
def _export_csvs(out_dir: Path, basename: str, precision: np.ndarray, recall: np.ndarray, support: np.ndarray, cm: np.ndarray, row_norm: np.ndarray) -> None:
...
def _run_one_model(cfg: DictConfig, report_cfg: DictConfig, entry: Dict[str, Any], device: torch.device) -> None:
    ...
    raw: Dict[str, Any] = torch.load(ckpt_path, map_location=device, weights_only=False)
    ...
    if out is not None and str(out).strip():
        ...
        _export_csvs(out_dir, _safe_filename(name), precision, recall, support, cm, row_norm)

@hydra.main(version_base=None, config_path='configs', config_name='config')
def main(cfg: DictConfig) -> None:
    report_cfg = cfg.get('report')
    if report_cfg is None:
        raise SystemExit('Missing report config. Example: python class_report.py +report=default')
