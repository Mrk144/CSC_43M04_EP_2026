...
class FocalLossWithSmoothing(nn.Module):
    ...
    def __init__(self, smoothing: float=0.1, gamma: float=2.0, class_weights: torch.Tensor | None=None) -> None:
        super().__init__()
    ...
    def forward(self, logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        if num_classes < 2:
            raise ValueError(f'FocalLossWithSmoothing requires num_classes >= 2 (got {num_classes}).')
