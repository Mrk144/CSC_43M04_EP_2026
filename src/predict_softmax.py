...
@torch.no_grad()
def _run_inference(model: torch.nn.Module, loader: DataLoader, device: torch.device, n_classes: int) -> Tuple[np.ndarray, np.ndarray]:
    model.eval()
    ...
    if softmax.shape[1] != n_classes:
        print(f'Warning: model output dim {softmax.shape[1]} != expected {n_classes}')
...
def main(cfg: DictConfig) -> None:
    ...
    np.savez(val_out, video_names=np.array(val_names), softmax=val_softmax.astype(np.float32), labels=val_labels.astype(np.int64), val_accuracy=np.float32(val_acc), model_name=str(ckpt.get('model_name', 'unknown')))
    ...
    test_dataset = VideoFrameDataset(root_dir=test_dir, num_frames=num_frames, transform=eval_transform, sample_list=test_samples)
    ...
    np.savez(test_out, video_names=np.array(test_names), softmax=test_softmax.astype(np.float32), model_name=str(ckpt.get('model_name', 'unknown')))
    print(f'Wrote {test_out}')
    summary_path = output_dir / 'summary.txt'
    summary_path.write_text(f"model_name={ckpt.get('model_name', 'unknown')}\ncheckpoint={checkpoint_path}\nval_clips={len(val_dataset)}  val_accuracy={val_acc:.4f}\ntest_clips={len(test_dataset)}\nnum_frames={num_frames}\n", encoding='utf-8')
