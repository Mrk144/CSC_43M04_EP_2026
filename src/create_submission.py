#!/usr/bin/env python3
"""
Run trained checkpoint(s) on the test split and write submission CSV(s)::
...
Four models at once (from repo root)::

    python src/create_submission.py +submission=multi_four_models

MoE on test (mean of expert logits)::

    python src/create_submission.py +moe=default moe.combination=mean

Uses the same Hydra layout as ``train.py`` / ``evaluate.py``.
"""
...
def _write_submission_csv(
    ...
    header: Tuple[str, ...] = ("video_name", "predicted_class"),
) -> None:
...
def run_inference(
    ...
    for batch_idx, (video_batch, _labels) in enumerate(loader, start=1):
        ...
        if batch_idx % log_interval == 0 or batch_idx == n_batches:
            print(
                f"{prefix}Inference batch {batch_idx}/{n_batches} "
...
def _predict_one_checkpoint(
    ...
    print(
        f"[{model_label}] Starting inference: {len(dataset)} clips, "
    ...
    if len(predictions) != len(video_names):
        raise RuntimeError(
            f"[{model_label}] Prediction count {len(predictions)} != {len(video_names)}"
        )
    return predictions


def _majority_vote(rows: List[List[int]]) -> List[int]:
    """Per-video majority over K model predictions (ties -> smallest class id)."""
    n = len(rows[0])
    out: List[int] = []
    for i in range(n):
...
def _run_multi_model_submission(
    cfg: DictConfig,
    video_names: List[str],
    video_dirs: List[Path],
    test_root: Path,
    ...
    for entry in models_cfg:
        ...
        all_preds[name] = _predict_one_checkpoint(
            ckpt_path, video_names, video_dirs, test_root, cfg, device, name
        )
    ...
    if bool(sub_cfg.get("write_per_model", False)):
        ...
        if not per_model_dir:
            raise ValueError(
                "submission.write_per_model=true requires submission.per_model_dir."
            )
        out_dir = Path(str(per_model_dir)).resolve()
        for name in names:
            _write_submission_csv(
                out_dir / f"submission_{name}.csv",
                video_names,
                all_preds[name],
            )
    ...
    if ensemble_mode == "majority":
        ...
        raise ValueError(
            "submission.ensemble_column=none requires output_format=wide "
            "(no single predicted_class to write)."
        )
    else:
        raise ValueError(
            f"Unknown submission.ensemble_column={ensemble_mode!r}; "
            "use majority | first"
        )

    combined_path = Path(
        str(sub_cfg.get("output_csv", cfg.dataset.submission_output))
    ).resolve()
    ...
    if output_format == "simple":
        ...
        print(
            ...
            f"Fichier à déposer : {combined_path}\n"
            f"(2 colonnes : video_name, predicted_class)\n",
            flush=True,
        )
        ...
        raise ValueError(
            f"Unknown submission.output_format={output_format!r}; use simple | wide"
        )


def _run_moe_submission(
    cfg: DictConfig,
    video_names: List[str],
    video_dirs: List[Path],
    test_root: Path,
    ...
    moe_cfg = cfg.moe
    out = Path(
        str(moe_cfg.get("output_csv", cfg.dataset.submission_output))
    ).resolve()
...
def _run_single_model_submission(
    cfg: DictConfig,
    video_names: List[str],
    video_dirs: List[Path],
    test_root: Path,
    ...
    predictions = _predict_one_checkpoint(
        checkpoint_path,
        video_names,
        video_dirs,
        test_root,
