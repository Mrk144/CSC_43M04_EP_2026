"""Soft-voting ensemble of pre-computed model predictions.

Pipeline:
1. Train each model -> ``best_model.pt``.
2. Run ``predict_softmax.py`` on each checkpoint -> ``val.npz`` + ``test.npz``
   inside the run's ``preds/`` directory.
3. List those npz pairs in a YAML config and run this script.

Available methods (cfg.ensemble.method):
- ``mean_softmax``     : plain arithmetic mean of softmax probabilities.
- ``weighted_softmax`` : weights from ``cfg.ensemble.weights`` (list) or
                          ``"auto"`` to use each model's val accuracy.
- ``temperature_then_mean`` : grid-search a per-model temperature on val
                          (minimize NLL), divide logits-equivalent by T, then
                          take the arithmetic mean. Same flavor as classic
                          model calibration before soft voting.

Run from src/:
    python ensemble.py +ensemble=default
"""
...
def _load_model_predictions(entry: Dict[str, Any]) -> Dict[str, Any]:
    """Load val/test npz for one model entry from the config."""
    ...
    return {
        "name": str(entry.get("name", val_npz.parent.parent.name)),
        "val_names": val["video_names"],
        "val_softmax": val["softmax"].astype(np.float32),
        "val_labels": val["labels"].astype(np.int64),
        "test_names": test["video_names"],
        "test_softmax": test["softmax"].astype(np.float32),
        "val_accuracy": float(val["val_accuracy"]) if "val_accuracy" in val.files else None,
    }


def _check_alignment(models: List[Dict[str, Any]]) -> Tuple[np.ndarray, np.ndarray]:
    """Ensure all models share the same val and test order; return both arrays."""
    val_names = models[0]["val_names"]
    test_names = models[0]["test_names"]
    for m in models[1:]:
        if list(m["val_names"]) != list(val_names):
            raise ValueError(
                f"val video order differs in model {m['name']!r} vs {models[0]['name']!r}"
            )
        if list(m["test_names"]) != list(test_names):
            raise ValueError(
                f"test video order differs in model {m['name']!r} vs {models[0]['name']!r}"
            )
...
def _resolve_weights(cfg_weights: Any, models: List[Dict[str, Any]]) -> np.ndarray:
    ...
    if cfg_weights == "auto":
        ...
        for m in models:
            if m["val_accuracy"] is None:
                raise ValueError(
                    f"Model {m['name']!r} has no val_accuracy in npz; cannot use weights=auto"
                )
    ...
    if isinstance(cfg_weights, (list, tuple)):
        if len(cfg_weights) != len(models):
            raise ValueError(
                f"weights length {len(cfg_weights)} != number of models {len(models)}"
            )
...
def _find_temperature(softmax: np.ndarray, labels: np.ndarray) -> float:
    """Grid-search T in a wide range and pick the one that minimizes NLL.

    Recovers approximate logits via ``log(softmax)`` (up to an additive
    constant, which softmax discards anyway).
    """
    eps = 1e-9
...
def main(cfg: DictConfig) -> None:
    ...
    if ens_cfg is None:
        raise SystemExit(
            "No 'ensemble' section found in config. Run with '+ensemble=default'."
        )
    ...
    best_individual = max(
        (m["val_accuracy"] or _top1_acc(m["val_softmax"], m["val_labels"]))
        for m in models
    )
    print(f"Best individual val accuracy:       {best_individual:.4f}")
    if val_acc < best_individual:
        print(
            "WARNING: ensemble is below the best individual model. "
            "Consider dropping the weakest model or switching method."
        )
