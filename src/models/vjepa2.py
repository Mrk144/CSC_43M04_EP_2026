...
Loads a HuggingFace V-JEPA2 checkpoint and exposes ``(B, T, C, H, W) -> (B, num_classes)``.

Default variant is fine-tuned on Something-Something v2 (SSv2), which matches
motion-centric clips in this challenge. The dataset serves 4 frames; we
interpolate to ``num_frames`` (16) inside ``forward``.

Variants
--------
- ``facebook/vjepa2-vitl-fpc16-256-ssv2`` (default; SSv2 fine-tuned, recommended)
- ``facebook/vjepa2-vitl-fpc16-256`` (encoder-only, no SSv2 head — legacy path)
- ``facebook/vjepa2-vitg-fpc16-256`` (ViT-g, heavier)
- ``facebook/vjepa2-vitl-fpc64-256`` (ViT-L, 64 frames, more temporal context)
"""
...
def _is_ssv2_classification_variant(variant: str) -> bool:
    """HF SSv2 fine-tunes ship with ``AutoModelForVideoClassification``."""
    v = variant.lower()
    return "ssv2" in v or v.endswith("-ssv2")


class VJEPA2Classifier(nn.Module):
    """V-JEPA2 video classifier (SSv2 HF head + 33-way replacement, or encoder + linear)."""

    def __init__(
        self,
        variant: str = "facebook/vjepa2-vitl-fpc16-256-ssv2",
        num_classes: int = 33,
