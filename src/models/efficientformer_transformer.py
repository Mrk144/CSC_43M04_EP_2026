"""
EfficientFormer V2 (timm) per-frame encoder + temporal transformer (Track A).

Expects inputs normalized with ImageNet mean/std via ``build_transforms``.
"""
...
try:
    ...
    raise ImportError(
        "efficientformer_transformer requires the 'timm' package. "
        "Install with: uv add timm"
    ) from e


class EfficientFormerTransformer(nn.Module):
    """Per-frame EfficientFormer V2 + lightweight temporal transformer."""
    ...
    def __init__(
        ...
        if self.d_model % num_heads != 0:
            raise ValueError(
                f"num_features ({self.d_model}) must be divisible by "
                f"num_heads ({num_heads}) for {variant!r}"
            )
        ...
        self.temporal_pos = nn.Parameter(
            torch.zeros(1, max(self.num_frames, 1), 1, self.d_model)
        )
        self.spatial_pos: Optional[nn.Parameter] = None
        if self.spatial_tokens > 1:
            self.spatial_pos = nn.Parameter(
                torch.zeros(1, 1, self.spatial_tokens, self.d_model)
            )
        ...
        if self.spatial_pos is not None:
            ...
            if spatial_pos.shape[2] != s:
                ...
                if side * side == spatial_pos.shape[2] and target_side * target_side == s:
                    ...
                    spatial_pos = (
                        grid.reshape(1, feat_c, target_side * target_side)
                        .permute(0, 2, 1)
                        .unsqueeze(0)
                    )
                else:
