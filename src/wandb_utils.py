...
def setup_wandb(cfg: DictConfig, run_dir: Path, model: Optional[Any]=None, *, job_type: str | None=None, name_prefix: str='') -> Any:
    wb = _wandb_cfg(cfg)
    ...
    try:
        ...
        print("wandb.enabled=true but package 'wandb' is not installed. Run: uv add wandb")
        return None
    name = wb.get('name')
    if not name:
    ...
    init_kw: Dict[str, Any] = {'project': str(wb.get('project', 'csc-what-happens-next')), 'name': str(name), 'config': OmegaConf.to_container(cfg, resolve=True), 'dir': str(run_dir), 'reinit': True}
...
def log_epoch(cfg: DictConfig, *, epoch: int, total_epochs: int, train_loss: float, train_acc: float, val_loss: float, val_acc: float, lr: float, best_val_accuracy: float, is_best: bool) -> None:
    ...
    if wandb.run is None:
        return
    step = epoch + 1
    wandb.log({'epoch': step, 'lr': lr, 'train/loss': train_loss, 'train/accuracy': train_acc, 'val/loss': val_loss, 'val/accuracy': val_acc, 'val/best_accuracy': best_val_accuracy, 'val/is_best': int(is_best)}, step=step)
