...
class TSMTwoStreamGated(nn.Module):
    ...
    def __init__(self, num_classes: int, num_frames: int=7, pretrained: bool=False, dropout_p: float=0.5, fold_div: int=8, backbone: BackboneName='resnet34') -> None:
        super().__init__()
        ...
        stream_kw = dict(num_classes=num_classes, backbone=backbone, num_frames=num_frames, pretrained=pretrained, dropout_p=0.0, fold_div=fold_div)
        ...
        self.gate = nn.Sequential(nn.Linear(feature_dim * 2, gate_hidden), nn.ReLU(inplace=True), nn.Linear(gate_hidden, feature_dim), nn.Sigmoid())
