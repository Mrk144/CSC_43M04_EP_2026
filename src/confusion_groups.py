...
def build_confusion_groups(cm: np.ndarray, support: np.ndarray, min_rate: float=0.1, min_pair_errors: int=12, min_group_size: int=2) -> List[Dict[str, object]]:
    n = cm.shape[0]
    parent = list(range(n))
    ...
    for class_ids in sorted(clusters.values(), key=lambda xs: (len(xs), min(xs))):
        ...
        groups.append({'class_ids': sorted(class_ids), 'top_pairs': [{'true': i, 'pred': j, 'count': c, 'rate': round(r, 4)} for (i, j, c, r) in pairs[:8]]})
    return groups
